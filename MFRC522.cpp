...
MFRC522::MFRC522(SPI_HandleTypeDef* hspi, GPIO_TypeDef* cs_port, uint32_t cs_pin, GPIO_TypeDef* reset_port, uint32_t reset_pin) {
	this->hspi = hspi;
...
{
  HAL_GPIO_WritePin(cs_port, cs_pin, GPIO_PIN_RESET); /* Select SPI Chip MFRC522 */
...
//  (void) m_SPI.write(value);
  HAL_SPI_Transmit(hspi, &cmd_w, 1, 500);
  HAL_SPI_Transmit(hspi, &value, 1, 500);
...
//  (void) m_SPI.write(reg & 0x7E);
  ...
  {
	  HAL_SPI_Transmit(hspi, &(values[index]), 1, 500);
...
//  (void) m_SPI.write(0x80 | reg);
  HAL_SPI_Transmit(hspi, &cmd_r, 1, 500);
